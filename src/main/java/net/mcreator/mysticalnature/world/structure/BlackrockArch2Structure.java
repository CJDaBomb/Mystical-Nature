...
import net.mcreator.mysticalnature.procedures.BlackrockArch2AdditionalGenerationConditionProcedure;
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if ((random.nextInt(1000000) + 1) <= 70000) {
					...
					for (int a = 0; a < count; a++) {
						...
						int y = spawnTo.getY();
						int z = spawnTo.getZ();
						if (!BlackrockArch2AdditionalGenerationConditionProcedure
								.executeProcedure(ImmutableMap.of("x", x, "y", y, "z", z, "world", world)))
							continue;
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("mystical_nature", "blackrock_arch_2"));
						if (template == null)
							return false;
