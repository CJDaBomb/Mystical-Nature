...
import net.mcreator.mysticalnature.procedures.BlackrockBoulder2AdditionalGenerationConditionProcedure;
...
public class BlackrockBoulder2Structure extends MysticalNatureModElements.ModElement {
	public BlackrockBoulder2Structure(MysticalNatureModElements instance) {
		super(instance, 111);
	}
		...
		Feature<NoFeatureConfig> feature = new Feature<NoFeatureConfig>(NoFeatureConfig::deserialize) {
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if (!dimensionCriteria)
					return false;
				if ((random.nextInt(1000000) + 1) <= 150000) {
					...
					for (int a = 0; a < count; a++) {
						...
						int y = spawnTo.getY();
						int z = spawnTo.getZ();
						if (!BlackrockBoulder2AdditionalGenerationConditionProcedure
								.executeProcedure(ImmutableMap.of("x", x, "y", y, "z", z, "world", world)))
							continue;
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("mystical_nature", "blackrock_boulder_2"));
						if (template == null)
							return false;
