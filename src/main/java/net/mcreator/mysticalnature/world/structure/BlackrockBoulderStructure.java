...
import net.mcreator.mysticalnature.procedures.BlackrockBoulderAdditionalGenerationConditionProcedure;
...
public class BlackrockBoulderStructure extends MysticalNatureModElements.ModElement {
	public BlackrockBoulderStructure(MysticalNatureModElements instance) {
		super(instance, 109);
	}
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if ((random.nextInt(1000000) + 1) <= 150000) {
					...
					for (int a = 0; a < count; a++) {
						...
						int y = spawnTo.getY();
						int z = spawnTo.getZ();
						if (!BlackrockBoulderAdditionalGenerationConditionProcedure
								.executeProcedure(ImmutableMap.of("x", x, "y", y, "z", z, "world", world)))
							continue;
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("mystical_nature", "blackrock_boulder"));
						if (template == null)
							return false;
