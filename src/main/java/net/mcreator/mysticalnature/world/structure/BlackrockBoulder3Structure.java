...
import net.mcreator.mysticalnature.procedures.BlackrockBoulder3AdditionalGenerationConditionProcedure;
...
public class BlackrockBoulder3Structure extends MysticalNatureModElements.ModElement {
	public BlackrockBoulder3Structure(MysticalNatureModElements instance) {
		super(instance, 113);
	}
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if ((random.nextInt(1000000) + 1) <= 150000) {
					...
					for (int a = 0; a < count; a++) {
						...
						int y = spawnTo.getY();
						int z = spawnTo.getZ();
						if (!BlackrockBoulder3AdditionalGenerationConditionProcedure
								.executeProcedure(ImmutableMap.of("x", x, "y", y, "z", z, "world", world)))
							continue;
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("mystical_nature", "blackrock_boulder_3"));
						if (template == null)
							return false;
