...
import net.mcreator.mysticalnature.procedures.AcidLake2AdditionalGenerationConditionProcedure;
...
public class AcidLake2Structure extends MysticalNatureModElements.ModElement {
	public AcidLake2Structure(MysticalNatureModElements instance) {
		super(instance, 104);
	}
			...
			public boolean place(IWorld world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
				...
				if ((random.nextInt(1000000) + 1) <= 15000) {
					...
					for (int a = 0; a < count; a++) {
						...
						int y = spawnTo.getY();
						int z = spawnTo.getZ();
						if (!AcidLake2AdditionalGenerationConditionProcedure
								.executeProcedure(ImmutableMap.of("x", x, "y", y, "z", z, "world", world)))
							continue;
						Template template = ((ServerWorld) world.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("mystical_nature", "acid_lake_2"));
						if (template == null)
							return false;
